...
//                    .append("}" + "&start=").append(offset).append("&limit=").append(LIMIT_FOR_HTTP_REQUEST_SIZE);
     ...
     */
    public static List<GestureNaiveBayes> getNaiveBayesGesturesFromDatabase(String name) throws IOException {
        Map<String, String> map = new HashMap<>();
        map.put("name", name);
    ...
    private static String determineDatabaseTable(Class<?> clazz) {
        ...
        if (clazz.getTypeName().equals(GestureNaiveBayesEulerDiffs.class.getTypeName())) {
            return SharedConstants.DB_TABLE_GESTURE_NAIVE_BAYES_EULER_DIFFS;
        }
